...
if ($matches) {
  Write-Host "Safety scan found terms. Review context below; V13.2 allows safety docs and negative statements only."
  $matches | Format-Table -AutoSize
} else {
  Write-Host "Safety scan found no matching terms."
}

Write-Host "Safety scan completed. No executable trade integration is expected in V13.2."
