...
if (-not (Test-Path -LiteralPath $python)) { throw "AlphaPilot virtualenv Python not found: $python" }
...
try {
  ...
  & $python -m alphapilot.reports.generate_v13_25_live_canary_report --code-commit $codeCommit
  if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
} finally {
  Pop-Location
}
