"""Remote publication gates for the V18.2 evidence-chain correction."""
...
from .v18_2_contracts import V18_2_TAG
...
def evaluate_v18_2_code_freeze(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    blockers: list[str] = []
    ...
    return {
        "schemaVersion": "s01_v18_2_remote_code_freeze_audit_v1",
...
def evaluate_v18_2_preregistration_freeze(
    snapshot: Mapping[str, Any], *, require_v18_2_tag: bool = True
) -> dict[str, Any]:
    blockers: list[str] = []
    ...
    freeze_tag_valid = bool(
        tags.get(V18_2_TAG) == snapshot.get("headCommit")
        or snapshot.get("v18_2TagIsAncestorOfHead") is True
    )
    if require_v18_2_tag and not freeze_tag_valid:
        blockers.append("remote_v18_2_tag_not_at_head")
    return {
        "schemaVersion": "s01_v18_2_remote_preregistration_freeze_audit_v1",
        "status": "blocked" if blockers else "passed",
        ...
        "v18_2TagRequired": require_v18_2_tag,
        "v18_2TagAtHead": tags.get(V18_2_TAG) == snapshot.get("headCommit"),
        "v18_2FreezeTagValid": freeze_tag_valid,
        "remoteTags": tags,
...
def audit_v18_2_code_freeze(
    *,
    ...
    if not executable:
        return evaluate_v18_2_code_freeze({})
...
def audit_v18_2_preregistration_freeze(
    *,
    repo_root: Path,
    preregistration_path: Path,
    git_executable: str | None = None,
    remote: str = "origin",
    require_v18_2_tag: bool = True,
) -> dict[str, Any]:
    root = Path(repo_root).resolve()
    ...
    executable = git_executable or shutil.which("git")
    if not executable:
        return evaluate_v18_2_preregistration_freeze({})
    ...
    v18_2_tag_commit = _remote_tag_commit(root, executable, remote, V18_2_TAG)
    return evaluate_v18_2_preregistration_freeze(
        {
            "headCommit": head,
        ...
        require_v18_2_tag=require_v18_2_tag,
    )
