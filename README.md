...
V13.4.20 turns the V13.4.19 coarse C/D research buckets into explicit
research-only probability gate candidate configs. It does not modify
`AlphaPilotDynamicRegimeV01.py`, does not modify the default probability gate,
does not run a backtest, does not approve Dry-run, and does not add live trading
capability.
...
```text
C1: coarse_c trend_medium_safe
D1: coarse_d trend_trend_module_low_safe
CD: combined C1 + D1 research candidate
```
...
Those rejected buckets remain diagnostic-only because they represent avoid,
unknown, or no-entry contexts. They are not entry candidates.

V13.4.20 keeps all candidate configs as:
...
The next version should be V13.4.21: Probability Gate Candidate Backtest. That
version should compare the current baseline gate against C1, D1, and CD on the
BTC / ETH / SOL smoke scope and then the historical dynamic universe expanded
scope.

## Next Versions

- V13.4.21: probability gate candidate comparative backtest, still research-only.
